...
export function AppLockProvider({ children }: { children: ReactNode }) {
  ...
  const [isResolving, setResolving] = useState(true); const [setupRequired, setSetupRequired] = useState(false); const [isLocked, setLocked] = useState(false); const [biometric, setBiometric] = useState<BiometricCapability | null>(null); const userIdRef = useRef<number | null>(null);
  useEffect(() => { let active = true; const load = async () => { if (isBootstrapping) return; setResolving(true); userIdRef.current = user?.id ?? null; if (!user) { if (active) { setSetupRequired(false); setLocked(false); setBiometric(null); setResolving(false); } return; } const [hasPin, capability] = await Promise.all([pinStorage.hasPin(user.id), getBiometricCapability().catch(() => null)]); if (!active || userIdRef.current !== user.id) return; await pinStorage.updateDisplayMetadata(user.id, user.fullName, maskedContact(user.phoneNumber, user.email)); if (active) { setSetupRequired(!hasPin); setLocked(hasPin); setBiometric(capability); setResolving(false); } }; void load(); return () => { active = false; }; }, [isBootstrapping, user]);
  ...
  const value = useMemo<AppLockValue>(() => ({ isResolving, setupRequired, isLocked, biometric, displayName:user?.fullName ?? "Foydalanuvchi", submitSetupPin, submitUnlockPin, unlockWithBiometrics, lockNow: () => setLocked(true) }), [biometric,isLocked,isResolving,setupRequired,submitSetupPin,submitUnlockPin,unlockWithBiometrics,user?.fullName]);
