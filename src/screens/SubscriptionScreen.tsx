  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      >
            ...
            <View style={styles.quotaRow}>
              <View style={styles.quotaItem}>
                <Text style={styles.quotaValue}>
                  {current.sms.totalRemaining === null ? "∞" : current.sms.totalRemaining}
                </Text>
              ...
              <View style={styles.quotaItem}>
                <Text style={styles.quotaValue}>
                  {current.unlimitedOrganizations ? "∞" : current.maxOrganizations ?? 0}
                </Text>
                <Text style={styles.quotaLabel}>{t("subscription.organizations")}</Text>
              </View>
              <View style={styles.quotaDivider} />
              <View style={styles.quotaItem}>
                <Text style={styles.quotaValue}>{current.blacklistEnabled ? "✓" : "—"}</Text>
        ...
        <Pressable
          accessibilityRole="button"
          accessibilityLabel={paymentCopy.history}
        ...
        >
          ...
          <View style={styles.flexCopy}>
            <Text style={styles.historyTitle}>{paymentCopy.history}</Text>
            <Text style={styles.historyHint}>{paymentCopy.historyHint}</Text>
          </View>
            ...
            return (
              ...
              >
                <View style={styles.planHeader}>
                  <View style={styles.flexCopy}>
                    <Text style={styles.planName}>{getPlanName(plan.code, plan.name)}</Text>
                ...
                <View style={styles.featureList}>
                  ...
                  />
                  <FeatureRow
                    text={plan.blacklistEnabled ? t("subscription.blacklistAvailable") : t("subscription.blacklistUnavailable")}
                    enabled={plan.blacklistEnabled}
                    theme={theme}
                    styles={styles}
                  />
                  <FeatureRow
                    text={plan.telegramBotEnabled ? t("subscription.telegramAvailable") : t("subscription.telegramUnavailable")}
                    enabled={plan.telegramBotEnabled}
                    theme={theme}
                    styles={styles}
                  />
                  <FeatureRow
                    text={plan.transactionSmsEnabled ? t("subscription.transactionSmsAvailable") : t("subscription.transactionSmsUnavailable")}
                    enabled={plan.transactionSmsEnabled}
                    theme={theme}
                    styles={styles}
                  />
                  <FeatureRow
                    text={plan.prioritySupportEnabled ? t("subscription.prioritySupportAvailable") : t("subscription.prioritySupportUnavailable")}
                ...
                {!isCurrent ? (
                  <Pressable
                    accessibilityRole="button"
                    accessibilityLabel={`${getPlanName(plan.code, plan.name)} ${plan.price === 0 ? paymentCopy.activatePlan : paymentCopy.buyPlan}`}
                    onPress={() =>
                  ...
                  >
                    <Text style={styles.planActionText}>
                      {plan.price === 0 ? paymentCopy.activatePlan : paymentCopy.buyPlan}
                    </Text>
...
function SectionTitle({ title, theme }: { title: string; theme: AppTheme }) {
  return (
    <Text style={{ ...typography.headingSmall, color: theme.text, marginTop: spacing.xs }}>
      {title}
    </Text>
  );
}
...
function FeatureRow({
  ...
  return (
    <View style={styles.featureRow}>
      ...
      />
      <Text style={[styles.featureText, !enabled && { color: theme.textMuted }]}>
        {text}
      </Text>
    </View>
  );
...
const createStyles = (theme: AppTheme) =>
  StyleSheet.create({
    ...
    title: { flex: 1, textAlign: "center", ...typography.headingLarge, color: theme.text },
    content: { padding: spacing.md, paddingBottom: spacing.xl, gap: spacing.sm },
    currentCard: { padding: 15, gap: 15, borderRadius: radius.xl, backgroundColor: theme.primaryLight, borderWidth: 1, borderColor: `${theme.primary}55` },
    currentTop: { flexDirection: "row", alignItems: "center", gap: 11 },
    planIcon: { width: 46, height: 46, alignItems: "center", justifyContent: "center", borderRadius: 15, backgroundColor: theme.surface },
    eyebrow: { ...typography.caption, color: theme.primary, fontWeight: "800", letterSpacing: 0.7 },
    currentName: { ...typography.headingMedium, color: theme.text },
    activeBadge: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: radius.full, backgroundColor: theme.paymentBg },
    activeBadgeText: { ...typography.caption, color: theme.paymentColor, fontWeight: "800" },
    quotaRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-around", paddingTop: 13, borderTopWidth: 1, borderTopColor: `${theme.primary}22` },
    ...
    quotaDivider: { width: 1, height: 30, backgroundColor: `${theme.primary}22` },
    historyRow: { minHeight: 62, flexDirection: "row", alignItems: "center", gap: spacing.sm, padding: spacing.sm, borderRadius: radius.lg, backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border },
    historyIcon: { width: 38, height: 38, borderRadius: 12, alignItems: "center", justifyContent: "center", backgroundColor: theme.primaryLight },
    historyTitle: { ...typography.bodyMedium, color: theme.text, fontWeight: "800" },
    historyHint: { ...typography.caption, color: theme.textSecondary },
    planCard: { padding: 15, gap: 10, borderRadius: radius.lg, backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border },
    planCardActive: { borderColor: theme.primary, backgroundColor: theme.surfaceElevated },
    ...
    currentLabel: { ...typography.caption, color: theme.primary, fontWeight: "800" },
    price: { ...typography.headingSmall, color: theme.primary },
    featureList: { gap: 7 },
    featureRow: { flexDirection: "row", alignItems: "center", gap: 8 },
    featureText: { ...typography.bodySmall, color: theme.textSecondary },
    planAction: { minHeight: 40, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7, marginTop: 2, borderRadius: radius.md, backgroundColor: theme.primaryLight },
    planActionText: { ...typography.labelSmall, color: theme.primary, fontWeight: "800" },
    loadingCard: { minHeight: 82, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 9, borderRadius: radius.lg, backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border },
    muted: { ...typography.bodySmall, color: theme.textMuted },
    packageGrid: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm },
    packageCard: { width: "48%", minHeight: 122, padding: 13, gap: 4, borderRadius: radius.lg, backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border },
    packageIcon: { width: 32, height: 32, alignItems: "center", justifyContent: "center", borderRadius: 10, backgroundColor: theme.primaryLight },
    packageName: { ...typography.label, color: theme.text, fontWeight: "800" },
    packagePrice: { ...typography.bodySmall, color: theme.primary, fontWeight: "700" },
    packageHint: { ...typography.caption, color: theme.textMuted },
    infoBox: { flexDirection: "row", gap: 8, padding: 12, borderRadius: radius.md, backgroundColor: theme.inputBackground },
